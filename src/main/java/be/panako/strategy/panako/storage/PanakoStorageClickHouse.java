 ...
 * <pre>
 * ALTER TABLE panako_metadata ADD COLUMN IF NOT EXISTS title Nullable(String);
 * ALTER TABLE panako_metadata ADD COLUMN IF NOT EXISTS audio_url Nullable(String);
 * </pre>
 */
public class PanakoStorageClickHouse implements PanakoStorage {
	...
	public void storeMetadata(long resourceID, String resourcePath, float duration, int fingerprints) {
		storeMetadataExt(resourceID, resourcePath, duration, fingerprints, null, null);
	}
	 ...
	 */
	public void storeMetadataExt(long resourceID, String resourcePath, float duration, int fingerprints,
								 String title, String audioUrl) {
		try (Connection conn = getConnection();
			 PreparedStatement ps = conn.prepareStatement(
					 "INSERT INTO panako_metadata (resource_id, path, duration, num_fingerprints, title, audio_url) VALUES (?, ?, ?, ?, ?, ?)")) {
	...
	/**
	 * Delete all rows for {@code resourceID} from both {@code panako_fingerprints}
	 * and {@code panako_metadata}. Uses ClickHouse lightweight {@code DELETE} which
	...
	public long deleteByResourceId(long resourceID) {
		try (Connection conn = getConnection()) {
			...
			try (PreparedStatement count = conn.prepareStatement(
					"SELECT count() FROM panako_fingerprints WHERE resource_id = ?")) {
			...
			try (PreparedStatement count = conn.prepareStatement(
					"SELECT count() FROM panako_metadata FINAL WHERE resource_id = ?")) {
				count.setLong(1, resourceID);
				try (ResultSet rs = count.executeQuery()) {
			...
			try (PreparedStatement del = conn.prepareStatement(
					"DELETE FROM panako_fingerprints WHERE resource_id = ?")) {
				del.setLong(1, resourceID);
				del.executeUpdate();
			}
			try (PreparedStatement del = conn.prepareStatement(
					"DELETE FROM panako_metadata WHERE resource_id = ?")) {
				del.setLong(1, resourceID);
				del.executeUpdate();
			}
			return fpCount + metaCount;
		} catch (SQLException e) {
			LOG.log(Level.SEVERE, "Failed to delete PANAKO resource_id=" + resourceID, e);
			return -1;
		}
	}
	...
	public PanakoResourceMetadata getMetadata(long identifier) {
		PanakoResourceMetadataExt ext = getMetadataExt(identifier);
	 ...
	 */
	public PanakoResourceMetadataExt getMetadataExt(long identifier) {
		try (Connection conn = getConnection();
			 PreparedStatement ps = conn.prepareStatement(
					 "SELECT resource_id, path, duration, num_fingerprints, title, audio_url FROM panako_metadata FINAL WHERE resource_id = ?")) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					if (rs.wasNull()) audioUrl = null;
					return new PanakoResourceMetadataExt(meta, title, audioUrl);
				}
			}
		} catch (SQLException e) {
